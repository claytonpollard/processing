...
/*Music: Strawberry Crisis by Kommisar (Press 1)
 *Music: Prism Stars by Derris Kharlan (Press 2)[Starting song btw]
*/
...
void setup() {
  ...
  player = minim.loadFile("Derris-Kharlan-Prism_Stars.mp3");  player.play();
}
...
void draw() {
  ...
  float averageAmplitude = sum / player.bufferSize();

  float brightnessValue = map(averageAmplitude, 0, 64, 0, 64);
  ...
  for (int y = 0; y < height; y += 10) {
    for (int x = 0; x < width; x += 10) {
      ...
      strokeWeight(strokeWeightValue*(averageAmplitude*20.));
...
void keyPressed() {
  if (key == '1') {
    player.close();
    player = minim.loadFile("Strawberry_Crisis1.wav");
    player.play();
  } else if (key == '2') {
    player.close();
    player = minim.loadFile("Derris-Kharlan-Prism_Stars.mp3");
    player.play();
  }
}
